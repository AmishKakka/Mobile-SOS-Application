  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        <Animated.View entering={ZoomIn.duration(600)} style={styles.iconRow}>
          <View style={styles.blueIcon}><Send color="#3B82F6" size={36} /></View>
          <View style={[styles.blueIcon, { marginLeft: -18, backgroundColor: '#DBEAFE', zIndex: -1 }]}><Camera color="#1D4ED8" size={36} /></View>
        </Animated.View>

        <Animated.Text entering={FadeInUp.delay(200)} style={styles.title}>Permissions Required</Animated.Text>
        <Animated.Text entering={FadeInUp.delay(300)} style={styles.desc}>
          SafeGuard needs access to your device location and camera to ensure total safety during an SOS event.
        </Animated.Text>

        <Animated.View entering={FadeInUp.delay(400)} style={styles.warningBox}>
          <AlertTriangle color="#B45309" size={26} />
          <View style={styles.wTextCol}>
            <Text style={styles.wTitle}>Background Location Access</Text>
            <Text style={styles.wDesc}>We require background access to ensure your coordinates are sent even if your screen is locked.</Text>
          </View>
        </Animated.View>

        <Animated.View entering={FadeInUp.delay(500)} style={styles.bulletList}>
        ...
        <Animated.View entering={FadeInUp.delay(600)} style={{ width: '100%' }}>
          ...
          <TouchableOpacity onPress={() => navigation.replace('MainDashboard')} style={styles.skipBtn}>
...
const styles = StyleSheet.create({
  ...
  blueIcon: { backgroundColor: '#EFF6FF', padding: 22, borderRadius: 100, elevation: 5, shadowColor: '#3B82F6', shadowOpacity: 0.15 },
  ...
  allowBtn: { backgroundColor: '#3B82F6', width: '100%', padding: 20, borderRadius: 18, alignItems: 'center', marginTop: 45, elevation: 6 },
